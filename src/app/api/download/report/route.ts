...
export async function GET(request: NextRequest) {
  try {
    ...
    // HTTP 헤더는 ASCII만 허용 → filename* 로 UTF-8 인코딩
    const asciiName = `SSW_report_${year}${month ? `_${String(month).padStart(2, "0")}` : ""}.xlsx`;
    const korName = `SSW_통계데이터_${year}${month ? `_${String(month).padStart(2, "0")}` : ""}.xlsx`;

    return new Response(buf, {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="${asciiName}"; filename*=UTF-8''${encodeURIComponent(korName)}`,
      },
    });
  } catch (err) {
