const baseUrl = process.env.V3_PRODUCTION_URL || "https://didongpul-dashboard-v3.vercel.app";
...
const apiPath = "/api/v3/dashboard/members?start_date=2026-06-14&end_date=2026-06-20&center=ALL";
const api = await fetchText(apiPath);
assert(api.response.status === 401, `비로그인 v3 API 응답이 401이 아닙니다: ${api.response.status}`);

let apiJson;
try {
  apiJson = JSON.parse(api.text);
} catch {
  throw new Error("v3 API가 JSON이 아닌 응답을 반환했습니다.");
}

assert(apiJson?.error === "인증이 필요합니다.", "v3 API 인증 오류 메시지가 예상과 다릅니다.");

console.log(`v3 production verification passed: ${baseUrl}`);
